...
Usage:
  python show_counts.py "OR99_RH_....TIF"
  python show_counts.py "..." --regions      # outline dNCM/vNCM/CMM (costs memory)
  python show_counts.py "..." --clicks       # overlay hand clicks too, if any exist
  python show_counts.py "..." --read-only    # look without being able to change anything
  python show_counts.py --check              # check the editing rules, no window, no image
"""
...
def main():
    ...
    if not Path(table_path).exists():
        raise SystemExit(
            ...
            f'  python count_cells.py "{image_path}" --points'
        )
