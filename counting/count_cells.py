...
Usage:
  python counting/count_cells.py                      # every bird with regions drawn (hours)
  python counting/count_cells.py "Gre595_....TIF"     # one image
  python counting/count_cells.py "..." --points       # also save every kept cell's position
  python counting/count_cells.py "..." --tta          # average 8 orientations (see classify)
  python counting/count_cells.py --check              # check the argument parsing only

  python counting/show_counts.py "..."                # then look at what --points saved
"""
...
# THE APP'S FOLDERS, BEFORE THE FIRST IMPORT THAT NEEDS THEM. This script runs as its own process --
# started by the app, or typed by hand -- so Python puts THIS folder on sys.path and none of its
# siblings, and `import ps6` below would fail. app_path.py at the root explains the arrangement; the
# line before it is there because app_path is itself at the root, which is not on the path yet
# either. The condition covers a flat copy of the app, where app_path sits right here.
...
# gives its subprocesses a working directory of the project root, so for the app nothing moves.
#
# THIS FILE'S FOLDER IS NOT WHERE THE WEIGHTS ARE ANY MORE. On 2026-09-17 this script moved into
# counting/, and the checkpoints stayed at the top level -- they are data, not code. So look beside
# this file FIRST (the exported app, where package_app.py puts the script and the model together in
# one flat folder) and in the parent SECOND (this folder). Beside-first, because the exported app is
# the copy that must not depend on anything above itself.
HERE = Path(__file__).resolve().parent


def model_path(name):
    """A checkpoint, beside this file if it is there and one folder up if it is not."""
    beside = HERE / name
    return str(beside if beside.exists() else HERE.parent / name)


MODEL_PATH = model_path("ps6_cnn_backbone.pt")
FALLBACK_MODEL_PATH = model_path("ps6_cnn.pt")
...
# drawn". So `python counting/count_cells.py --help` did not print help. It started a full-dataset
...
def find_images(argument):
    ...
    if not images:
        raise SystemExit(
            ...
            "  python regions/draw_regions.py"
        )
...
def parse_arguments(argv):
    ...
    if len(images) > 1:
        raise SystemExit(
            ...
            '  python counting/count_cells.py "Gre595_RH_3-1-4_NCM_Slide 1.TIF" --points'
        )
...
def check_arguments():
    ...
    Run with: python counting/count_cells.py --check
    """
