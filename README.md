...
You need **one** of these installed first — either is enough, and the launcher does the rest:
...
* **[uv](https://docs.astral.sh/uv/)** — if you have uv but no suitable Python, the launcher uses it
  to fetch a private Python of its own. This needs no admin, and nothing lands on the system.
...
2. Double-click **`Count cells.command`** on macOS, or **`Count cells.bat`** on Windows. On Linux,
   run `bash count-cells.sh`. On macOS you can also run `./"Count cells.command"` from a terminal.
