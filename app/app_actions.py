...
Usage:
  python app/app_actions.py                          # show what each section would run
  python app/app_actions.py --draw "<stem or path>"   # open the drawing window
  python app/app_actions.py --count "<stem or path>"  # count one section
  python app/app_actions.py --review "<stem or path>" # look at the cells it counted
  python app/app_actions.py --areas "<stem or path>"  # recompute its cached areas
"""
...
# THE PROJECT ROOT, which is NOT this file's folder any more. On 2026-09-17 the 64 supporting
# scripts moved into app/ regions/ review/ training/ counting/, so `HERE` is `.../cell counting/app`
# here and `.../cell counting` in the exported app, where package_app.py still copies everything
# flat beside app.py. One rule covers both: the root is the folder that holds app.py.
#
# This matters twice below. `cwd=ROOT` starts every child in the project folder, because the files
# they read and write -- ps6_counts.csv, the sections, the sidecars -- are named relative to it.
# Starting a count in `app/` would scatter those into `app/`. And script() has to look in the
# subfolders, because the scripts it names are no longer neighbours of this file.
ROOT = HERE if (HERE / "app.py").exists() else HERE.parent

# Where script() looks, in order. The root first so the flat exported app never searches at all,
# then the five role folders. Deliberately a list of names and not a glob: a scan of every folder
# under the root would eventually find a same-named script in `old/` or `archive/` and run it.
SCRIPT_FOLDERS = ["app", "regions", "review", "training", "counting"]
...
def script(name):
    """One of the app's own scripts, by absolute path. See ROOT.

    Two layouts have to work from the same code. In THIS folder the scripts are sorted into
    app/ regions/ review/ training/ counting/; in the app that goes to somebody else they are
    all flat beside app.py, because package_app.py copies them that way on purpose -- a
    stranger unzipping a folder should see the files, not a tree.

    Falls back to ROOT / name when nothing is found, so the error a caller gets is the ordinary
    "can't open file '.../count_cells.py'" naming a real path, rather than a None from here
    turning into something harder to read further down.
    """
    if (ROOT / name).exists():
...
def run(command, on_output=None, on_start=None):
    ...
    process = subprocess.Popen(
        ...
        # or, on Windows, into C:\. ROOT and not HERE since the 2026-09-17 move: this file lives in
        # app/ now, and a count started in app/ would write ps6_counts.csv there.
        cwd=ROOT,
    )
...
def main():
    ...
    print(
        ...
        f"  python app/app_actions.py --count {sections[0]['stem'][:28]!r}"
    )
