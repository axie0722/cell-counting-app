...
Usage:
  python regions/draw_regions.py                     # first bird in ps6.BIRDS
  python regions/draw_regions.py "Gre595_....TIF"    # a specific image
  python regions/draw_regions.py Gre595_RH_3-1-4     # any unique fragment, either folder
  python regions/draw_regions.py Gre595_RH_3-1-4 --hp   # with the hippocampus layer (press 5)
"""
...
# the top of count_cells.py: this runs as its own process, so it gets regions/ on sys.path and none of
# its siblings.
...
# behind: everything below runs the moment it is read, which is what makes `python regions/draw_regions.py`
...
if __name__ != "__main__":
    raise ImportError(
        ...
        'a section\'s outlines. Run it as `python regions/draw_regions.py "<section>.TIF"`, or let the app '
...
# that tool, which is not what was asked. `python regions/draw_regions.py <image> --hp` still has it,
...
if arguments:
    ...
    if not Path(image_path).exists():
        raise SystemExit(
            'Usage: python regions/draw_regions.py "<section>.TIF"\n\n'
