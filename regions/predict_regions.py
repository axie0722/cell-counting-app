...
Usage:
  python regions/predict_regions.py "Gre595_RH_3-1-4_NCM_Slide 1.TIF"     # write nothing, just report
  python regions/predict_regions.py Gre595_RH_3-1-4 --check               # score against her own outlines
"""
import argparse
import json
...
#
# STILL CORRECT AFTER THE 2026-09-17 MOVE because the JSON moved too: this file is in regions/ and so
# is region_constants.json. It is the only data file that belongs to the code rather than to the
# folder, which is why it is the only one that followed a script into a subfolder.
